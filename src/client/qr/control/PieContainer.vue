<template>
  <ControlWrapper v-if="shouldShowControl" :control="control" :index="index">
    <PieChart v-if="finalSegments.length > 0 && sum > 0"
...
export default {
  ...
  created() {
    ...
    if (!this.control.complete) {
      console.error(`Pie control[${this.control.id}] is not complete.`)
      this.loaded = true;
      return;
    }
    ...
    presentationApi.fetchPresentation(this.qrId, this.currentPageId, this.control.id)
        ...
        .catch(ex => {
          console.error("Failed to fetch pie segment data:" + ex.message);
