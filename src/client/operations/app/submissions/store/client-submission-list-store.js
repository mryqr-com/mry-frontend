import ACTIONS from '@/client/operations/app/submissions/store/client-submission-list-actions';
...
const getters = {
    ...
    hasFilter(state) {
        return (state.initGroupId === 'ALL' && state.groupId !== 'ALL') ||
            (state.originPageId === 'ALL' && state.pageId !== 'ALL') ||
...
const actions = {
    [ACTIONS.LIST_SUBMISSIONS]({commit, state}, infiniteState) {
        let filterables = {};
        ...
        return submissionApi.listSubmissions(queryCommand, {showLoading: false}).then((response) => {
            let payload = {
...
const mutations = {
    ...
    initState(state, {appId, originGroupId, groupId, pageId, type}) {
        state.appId = appId;
