<template>
  <el-dialog :class="$style.wrapper"
    ...
    <el-form ref="theForm"
      ...
      <section :class="$style.aliasSection">
        <div :class="$style.aliasNote">
          ...
          <span :class="$style.sampleAlias">age</span>，那么在表达式中应该使用<span :class="$style.sampleAlias">#age</span>进行引用。更多关于自动计算的解释请<a
            class="primaryColoredTextLink" href="//docs.mryqr.com/page-controls/item-status/#自动计算"
            target="_blank">参考状态选择控件文档</a>。
        </div>

        <div id="itemStatusAliasContainer" :class="$style.aliasContainer">
      ...
      <section :class="$style.recordSection">
        <div>规则设置：</div>
        <div id="itemStatusRecordsContainer" :class="$style.recordsContainer">
          <div v-for="(record,index) in localValue.records" :key="record.id" :class="$style.recordRow">
            <el-form-item :class="$style.recordName"
                          :prop="'records.' + index + '.expression'"
                          :rules="{required: true, message: '表达式不能为空', trigger: 'change'}">
              <el-input v-model="record.expression"
                        :maxlength="200"
                        :spellcheck="false"
                        placeholder="请输入表达式"
                        size="mini"
            ...
            <span :class="$style.comma">：</span>
            <el-form-item :class="$style.recordOption"
                          :prop="'records.' + index + '.optionId'"
                          :rules="{ required: true, message: '选项不能为空', trigger: 'change'}"
                          title="表达式为真时所对应的选项">
              <el-select v-model="record.optionId"
                         :class="$style.controlSelect"
                         placeholder="请选择"
                         size="mini">
                <el-option
...
export default {
  ...
  methods: {
    ...
    addAlias() {
      ...
      this.$nextTick(() => {
        let element = document.getElementById("itemStatusAliasContainer");
        element.scrollTop = element.scrollHeight;
      });
    },

    addRecord() {
      if (this.localValue.records.length >= 20) {
        this.$message({
          type: 'error',
          message: `添加失败，最多添加20个规则项。`,
          center: true,
          duration: 1000,
        });
        return;
      ...
      this.localValue.records.push({id: uuid.generate(), expression: null, optionId: null});

      this.$nextTick(() => {
        let element = document.getElementById("itemStatusRecordsContainer");
        element.scrollTop = element.scrollHeight;
      });
    },
    ...
    deleteRecord(id) {
      this.localValue.records.splice(this.localValue.records.findIndex(i => i.id === id), 1);
    },
    ...
    confirm() {
      if (this.localValue.records.length !== 0 && this.localValue.aliasContext.controlAliases.length === 0) {
        this.$message({
          type: 'error',
          message: `请添加控件别名。`,
          center: true,
