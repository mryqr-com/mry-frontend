<template>
  <el-dialog :title="isNew? `新建属性值分时统计图`:`编辑属性值分时统计图`"
    ...
    <el-form v-if="internalReport.setting"
      ...
      <section :class="$style.settingSection">
        <div v-for="(setting,index) in internalReport.setting.segmentSettings"
          ...
          <el-form-item v-if="setting.basedType && setting.basedType !== 'CREATED_AT'"
                        :prop="'setting.segmentSettings.' + index + '.basedAttributeId'"
                        :rules="[{required: true, message: '请选择基准属性项', trigger: 'change'}]"
                        label="基准属性项：">
            <el-select v-model="setting.basedAttributeId"
                       :class="$style.fieldInput"
                       placeholder="请选择基准属性项"
                       size="medium"
                       title="所引用的基准属性项">
              <el-option v-for="attr in allDateableAttributes"
                         :key="attr.id"
                         :label="attr.name"
                         :value="attr.id">
              </el-option>
          ...
          <el-form-item v-if="setting.segmentType && setting.segmentType !== 'QR_COUNT_SUM'"
                        :prop="'setting.segmentSettings.' + index + '.targetAttributeId'"
                        :rules="[{required: true, message: '请选择目标属性项', trigger: 'change'}]"
                        label="目标属性项：">
            <el-select v-model="setting.targetAttributeId"
                       :class="$style.fieldInput"
                       placeholder="请选择目标属性项"
                       size="medium"
                       title="所引用的目标属性项">
              <el-option v-for="attr in allNumberedAttributes"
                         :key="attr.id"
                         :label="attr.name"
                         :value="attr.id">
              </el-option>
      ...
      <el-form-item ref="intervalItem" label="时段间隔：" prop="setting.interval">
        <el-select v-model="internalReport.setting.interval"
                   ...
                   size="medium"
                   title="时段间隔">
          <el-option v-for="interval in intervals"
                     :key="interval.key"
                     :label="interval.name"
                     :value="interval.key">
          </el-option>
  ...
  computed: {
    ...
    basedTypes() {
      return [
        {
          key: 'CREATED_AT',
          name: `按${this.instanceDesignation}的添加时间统计`
        },
        {
          key: 'DATE_ATTRIBUTE',
          name: '按时间属性项统计'
