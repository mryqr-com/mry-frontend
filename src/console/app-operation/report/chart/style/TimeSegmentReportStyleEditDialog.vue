<template>
  <el-dialog :title="`样式设置:【${report.name}】`"
    ...
    <el-form v-if="internalReport.style"
      ...
      <el-form-item ref="colorItem" label="颜色：" prop="style.color">
        <MryColorPicker v-for="index in settingCount"
                        :key="index"
...
export default {
  ...
  computed: {
    ...
    settingCount() {
      if (this.report.type === 'CONTROL_TIME_SEGMENT_REPORT') {
        return this.report.setting.segmentSettings.length;
      } else {
        return this.report.setting.segmentSettings.length;//control和attribute的结构相同，但这种相同时偶然性的，因此即便相同也分开处理
      }
    },
  },
